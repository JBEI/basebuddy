...
def get_cocoput_organism_list():
  ...
  return df.apply(
    lambda r: f"{r['Species']} (TaxID: {r['Taxid']})", axis=1
  ).unique().tolist()
...
st.set_page_config(page_title="Recoder")
st.header('''Recoder''')
st.write('''
...
if optimization_method == 'harmonize_rca':
  ...
  codon_optimize_kwargs = {
    'original_codon_usage_table': source_coding_table
  }
else:
  codon_optimize_kwargs = {}
...
try:
  for record in records:
    problem = DnaOptimizationProblem(
        ...
        objectives=[CodonOptimize(method=optimization_method, codon_usage_table=target_coding_table, **codon_optimize_kwargs)]
    )
